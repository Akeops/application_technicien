	...
	var clientsList = listing([true, false, false, false]);
	...
	function refresh(){
		...
		if(document.getElementsByName("CLIENT")[0].value != "" && clientID != 0){
	...
	function newClient(){
		document.getElementsByName("CLIENT")[0].value = document.getElementById("searchingClient").value;
		document.getElementsByName("CODE_CLIENT")[0].value = "NEW"; // CODE CLIENT
	...
	function refreshDescTab(){
		var temp = document.getElementsByName('DESCRIPTION')[0].value;
		var count = (temp.match(/\n/g) || []).length;
		document.getElementsByName('descTab')[0].rows = count + 1;
		document.getElementsByName('DESCRIPTION')[0].rows = count + 1;
		
		document.getElementsByName('descTab')[0].value = document.getElementsByName('DESCRIPTION')[0].value;
	}
	...
	function refreshDesc(){
		...
		document.getElementsByName('DESCRIPTION')[0].value = document.getElementsByName('descTab')[0].value;
	}
	
	function refreshComplementsTab(){
		var temp = document.getElementsByName('COMPLEMENTS')[0].value;
		var count = (temp.match(/\n/g) || []).length;
		document.getElementsByName('complementsTab')[1].rows = count + 1;
		
		document.getElementsByName('complementsTab')[1].value = document.getElementsByName('COMPLEMENTS')[0].value;
	}
	...
	function refreshComplements(){
		...
		var count = (temp.match(/\n/g) || []).length;
		document.getElementsByName('complementsTab')[1].rows = count + 1;
		document.getElementsByName('COMPLEMENTS')[0].rows = count + 1;
		
		document.getElementsByName('COMPLEMENTS')[0].value = document.getElementsByName('complementsTab')[1].value;
		document.getElementById("pendingReason").value = document.getElementsByName('COMPLEMENTS')[0].value;
	}
	...
	function materiel(state){
		...
		var output = document.getElementsByName("PRET");
		var obj = document.getElementsByName("OBJET");
	...
	function agence(entry){
		...
		if(sw){
			document.getElementsByName(entry)[0].value = "BAYONNE";
		}
		else{
			document.getElementsByName(entry)[0].value = "FLOIRAC";
		}
	}
	...
	function contrat(entry){
		...
		if(sw){
			document.getElementsByName(entry)[0].value = "false";
		}
		else{
			document.getElementsByName(entry)[0].value = "true";
		}
	}
	...
	function calcCharges(){
		...
		if(contrat == "true"){
			...
			document.getElementById("total").value = ttc + " \u20AC";
		}
	}
	...
	function formationDist(){
		document.getElementsByName("SIGNATURE")[0].value = "http://sav.tacteo.fr/signatures/.formation_distance/formationDist.png";
	}
	...
	function saved(){
		...
		var nomClient = document.getElementsByName("CLIENT")[0].value;
		...
		$.ajax({
			...
			var result = document.getElementsByName("SIGNATURE")[0];
	...
	function checking(state){
		...
		AGENCE =	 		document.getElementsByName("AGENCE")[0]; 	//01
		OBJECT = 			document.getElementsByName("OBJECT")[0]; 	//02
		HOUR_START =	 	[document.getElementById("hourArrived"), document.getElementById("minuteArrived")]; 	//03
		SEL_CLIENT = 		document.getElementsByName("CLIENT")[0]; 	//04
		CODE =	 			document.getElementsByName("CODE_CLIENT")[0]; 	//05
		ADDRESS =	 		document.getElementsByName("ADRESSE")[0];	//06
		TELEPHONE = 		document.getElementsByName("TELEPHONE")[0]; //07
		MAIL = 				document.getElementsByName("EMAIL")[0]; 	//08
		CONTRAT = 			document.getElementsByName("CONTRAT")[0]; //09
		DESC = 				document.getElementsByName("DECRIPTION")[0]; //10
		ZONE = 				document.getElementsByName("DEPLACEMENT")[0]; 	//11
		PRET = 				document.getElementsByName("PRET")[0]; //12
		MAT = 				[document.getElementsByName("MATERIELS")[0], document.getElementsByName("entry.2047563043")[0]]; //13
		TEST = 				document.getElementsByName("FONCTIONNEMENT")[0]; 	//14
		SAVE = 				document.getElementsByName("FORMATION_SAUVEGARDE")[0]; //15
		LOI = 				document.getElementsByName("LOI_NF")[0]; //16
		MAJ = 				document.getElementsByName("MISE_A_JOUR")[0]; 	//17
		VERSION = 			document.getElementsByName("VERSION")[0]; //18
		COMPLEMENTS = 		document.getElementsByName("COMPLEMENTS")[0];	//19
		HOUR_END = 			[document.getElementById("hourLeaved"), document.getElementById("minuteLeaved")]; //20
		NAME = 				document.getElementsByName("SIGNATAIRE")[0];	//21
		QUALITY = 			document.getElementsByName("QUALITY")[0]; 	//22
		TECH = 				document.getElementsByName("TECHNICIEN")[0]; //23
		SIGN = 				document.getElementsByName("SIGNATURE")[0];	//24
		CODECOM =			document.getElementsByName("COMMERCIAL")[0];
		...
		if(HOUR_START[0].value > HOUR_END[0].value 		&& (state == "HOUR"			|| state == "*")){
			if(confirm("ATTENTION : Les heures d'arrivée/départ sont mal renseignées. Continuer ?")){
			}
			else {
				show("HOUR_START");
				question = 3;
